export const SET_GRID = "SET_GRID";
export const GET_GRID = "GET_GRID";
export const SET_STARTING_ROW = "SET_STARTING_ROW";
export const GET_STARTING_ROW = "GET_STARTING_ROW";
export const SET_STARTING_COL = "SET_STARTING_COL";
export const GET_STARTING_COL = "GET_STARTING_COL";
export const SET_FINISH_ROW = "SET_FINISH_ROW";
export const GET_FINISH_ROW = "GET_FINISH_ROW";
export const SET_MOUSE_IS_PRESSED = "SET_MOUSE_IS_PRESSED";
export const GET_MOUSE_IS_PRESSED = "GET_MOUSE_IS_PRESSED";
